...
class DeribitREST:
    ...
    get_trades_by_time_range — paginated historical trades over an arbitrary window
    """
    ...
    async def get_last_trades(
        ...
        return [self._parse_trade(instrument_name, t) for t in r["trades"]]

    async def get_trades_by_time_range(
        self,
        instrument_name: str,
        ...
        """
        Fetch every public trade in [start_timestamp_ms, end_timestamp_ms), paginating
        public/get_last_trades_by_instrument_and_time forward in time.

        This is Deribit's only historical trade endpoint -- there is no free
        historical L2/order-book endpoint. A backtest built on this has real
        prices, sizes, and aggressor sides, but no real book depth or queue
        position; see core/backtest/historical.py for how that limitation is
        handled explicitly rather than silently assumed away.
        """
        trades: list[Trade] = []
        cursor = start_timestamp_ms
        while cursor < end_timestamp_ms:
            r = await self._get("public/get_last_trades_by_instrument_and_time", {
                "instrument_name": instrument_name,
            ...
            if not page:
                break
            trades.extend(self._parse_trade(instrument_name, t) for t in page)
            last_ts = int(page[-1]["timestamp"])
            if last_ts <= cursor:
                break  # guard against a non-advancing cursor on a malformed page
            cursor = last_ts + 1
            if not r.get("has_more", len(page) == page_size):
                break
        return trades

    @staticmethod
    def _parse_trade(instrument_name: str, t: dict[str, Any]) -> Trade:
        return Trade(
            instrument_name=instrument_name,
            price=float(t["price"]),
