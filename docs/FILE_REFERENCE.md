...
| `paper/engine.py` | `PaperTradingEngine` — wires book updates to quoting (perp via `core.strategies.market_maker` with a soft-hedge inventory override, option via `paper.option_quoting`), trade updates to queue-tracker fills (latency-gated), portfolio Greeks refresh + hard-hedge check on every update, and session logging. Positions keyed by `exchange:market_type:symbol`, not bare symbol — Binance spot and perpetual share the literal symbol `"BTCUSDT"` | active |
| `paper/dashboard.py` | `render_dashboard()` — refreshing terminal table: market mid, our quotes, position, unrealized P&L (labelled by currency), plus a portfolio-Greeks-per-underlying section and hard-hedge count | active |
...
| `tests/test_dashboard.py` | pytest coverage for `paper/dashboard.py`: formatting, no scientific notation, coin-denominated P&L labels, portfolio-Greeks section, hard-hedge count | active |
