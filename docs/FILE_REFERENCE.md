...
| `ccxt_stream/select_option.py` | `select_near_1dte_option()` — nearest-to-1-day-to-expiry, nearest-the-money Deribit call, via ccxt REST. `select_strikes_near_expiry()` — same expiry selection, but returns the `n_strikes` nearest-to-spot calls at that expiry instead of just one, the cross-section `core.models.options.surface_quoting.fit_smile()` needs to fit an SVI smile. Both filter to `settle == currency` (coin-settled/inverse) — Deribit also lists USDC-settled options on the same strike/expiry, which would otherwise show up as look-alike duplicate instruments and don't match this project's inverse-option quoting convention. Binance discontinued its options market in 2024 (verified live: zero option markets listed) so Deribit — already this project's real options venue per CLAUDE.md — is the only live source for "1DTE options" data | active |
| `ccxt_stream/record.py` | Runnable recorder: 25-level Binance USDT-M perpetual + **spot** (BTC, ETH — spot is CLAUDE.md's second reference/hedge leg, not previously streamed) depth + trades, and a cross-section of `n_strikes` (default 5) Deribit calls nearest spot at the nearest-1DTE expiry per coin, depth + trades. Runs *indefinitely* in rotating sessions (default 1h) rather than one long-lived connection — each session re-selects the option universe (1DTE options expire daily; a stale symbol would silently go quiet with no error), writes its own file, and an uncaught exception in one session is caught by the outer loop and logged/retried rather than killing the recorder (the individual ccxt.pro connectors already retry forever on transient WS errors internally). Writes to `runtime/ccxt_recordings/*.jsonl` via `data/recorder.py`. Run: `python ccxt_stream/record.py [session_seconds] [n_strikes]`, or under `caffeinate -i nohup ... &` for unattended multi-day capture (see module docstring) | active |
